...
def make_gasoil_endpoints(show=True, krgendanchor="sorg"):
    ...
    axes.annotate(
        "", xy=(-0.01, 0.04), xytext=(sgcr, 0.04), arrowprops={"arrowstyle": "<->"}
    )
...
def make_wateroil_paleooil_idc1(show=True):
    ...
    plt.xlabel("SW", labelpad=-10)
    axes.annotate(
        "", xy=(1 - sorw, krwend), xytext=(1, krwend), arrowprops={"arrowstyle": "<->"}
    )
...
def make_gaswater_paleogas_dci3(show=True):
    ...
    axes.annotate(
        "", xy=(1 - sgrw, krwend), xytext=(1, krwend), arrowprops={"arrowstyle": "<->"}
    )

    axes.text(1 - sgcr + 0.04, 0.09, "SGCR")
    axes.annotate(
        "", xy=(1 - sgcr, 0.07), xytext=(1, 0.07), arrowprops={"arrowstyle": "<->"}
    )

    axes.text(1 - sgl + 0.02, 0.02, "SGL")
    axes.annotate(
        "", xy=(1 - sgl, 0.00), xytext=(1, 0.00), arrowprops={"arrowstyle": "<->"}
    )
